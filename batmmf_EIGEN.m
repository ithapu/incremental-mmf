...
%% eigen MMF code -- batch

function [Us, Wids, Sids, Acomps, H, L, time] = batmmf_EIGEN(A, k, file)
%
tic;
fprintf('\n ========= \n Eigen-based batch Higher-order MMF (Parallel Operations) \n ========= \n');
%
...
for l = 1:1:L
    ...
    Es_I = zeros(1,size(combs,1)); bestOrth = cell(size(combs,1),1);
    wavepos_ind = zeros(1,size(combs,1));
    %
    evalc('parpool(''local'',8)');    
    parfor c = 1:1:size(combs,1)
        ...
        [Es_I(1,c), wavepos_ind(1,c)] = min(Es_c);
        bestOrth{c,1} = V;
    end
    evalc('delete(gcp(''nocreate''))');    
    [~, bestcomb_ind] = min(Es_I);     
    %
    %% set the wavelets and corresponing orth transform U_l
    U_l = eye(n,n); I_l = combs(bestcomb_ind,:); O_l = bestOrth{bestcomb_ind,1};
    for j1 = 1:1:k
        for j2 = 1:1:k
    ...
    A_l = U_l*A_lprev*U_l';
    pickW = wavepos_ind(bestcomb_ind); % the wavelet
    %
