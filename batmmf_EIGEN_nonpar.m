...
%% eigen MMF code -- batch (No parallel)

function [Us, Wids, Sids, Acomps, H, L, time] = batmmf_EIGEN_nonpar(A, k, file)
%
tic;
fprintf('\n ========= \n Eigen-based batch Higher-order MMF (No Parallel Operations) \n ========= \n');
%
...
for l = 1:1:L
    ...
    Es_I = Inf; bestcomb_ind = 0; wavepos_ind = 0; bestOrth = eye(k);    
    %
    for c = 1:1:size(combs,1)
        ...
        if min(Es_c)<Es_I
           [Es_I, wavepos_ind] = min(Es_c); bestOrth = V; bestcomb_ind = c;
        end
    end    
    %
    %% set the wavelets and corresponing orth transform U_l
    U_l = eye(n,n); I_l = combs(bestcomb_ind,:); O_l = bestOrth;
    for j1 = 1:1:k
        for j2 = 1:1:k
