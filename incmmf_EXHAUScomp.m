...
%% ideal MMF code -- ONLINE

function [Us_o, Wids_o, Sids_o, Acomps_o, H_o, L_o, time_o] = incmmf_EXHAUScomp(C, k, complete, comp_opt, lossy_or_lossless, file)
%
tic;
fprintf('\n ========= \n Incremental Higher-order MMF \n');
fprintf(' (Completion is Exhaustive-based; Initialization is user-defined) \n ========= \n');
%
...
for r = 1:1:length(rest)
    ...
    for l = 1:1:L
        ...
        newcombs_errs = zeros(num_total_combs,1);
        min_ind_new_list = zeros(num_total_combs,1);
        new_wave_pos_list = zeros(num_total_combs,1);
        %
        ...
        for idlen = 1:1:num_total_combs
            ...
            %
            [newcombs_errs(idlen), q] = min(reshape(Es_c, 1, []));
            [min_ind_new_list(idlen), new_wave_pos_list(idlen)] = ind2sub(size(Es_c), q); clear q;
        end
        %
        ...
        if err_new >= err_old
            ...
            insert(1,l) = 1;
            [~, id_update_best] = min(newcombs_errs);
            %[knew_best, id_update_best] = ind2sub(size(newcombs_errs), q); clear q;
            min_ind_new = min_ind_new_list(id_update_best); 
            new_wave_pos = new_wave_pos_list(id_update_best);
            %
            % the new combination
            newcombs = zeros(1,k); newcombs(1,end) = total_combs(id_update_best,new_wave_pos);
            newcombs(1:end-1) = total_combs(id_update_best,setdiff(1:1:k,new_wave_pos));
            % update the rotation
